...
# Usage:
#
# $ remove-comments.sed < input.c
# $ remove-comments.sed < input.c > output.c
# $ tmp=$(mktemp); find ./ -name "*.[ch]" | while read -r f; do echo $f; remove-comments.sed < "$f" > $tmp && mv $tmp "$f"; done
#
# Source:
